...
export function renderClimb(el) {
  ...
  RS.forEach((r) => {
    const c = commitFor(r.id, w.prevKey), a = autoDid(cfg, opps, r.id, w.prevKey), tc = commitFor(r.id, w.key);
  ...
  const teams = teamTabs().map((t) => {
    ...
    if (!rr.length) return '';
    return `
    <div class="sec"><div class="sec-h"><h2 class="s16">${esc(t.title || t.label)}${t.team_label ? ' · ' + esc(t.team_label) : ''}</h2><span class="sub">last week did / committed · this week committed</span></div>
    ...
    ${rr.map((r) => {
      ...
      return `<tr><td><span class="person">${esc(r.full_name)}</span></td><td>${esc(r.branch)}</td>${cfg.measures.map((m) => {
        ...
        return `<td class="num"><span class="dot ${c ? ryg(did, com) : 'n'}"></span> ${fmtM(m, did)}<small class="m">/${fmtM(m, com)}</small><br><small class="i">→ ${fmtM(m, tcv)}</small></td>`;
  ...
  el.innerHTML = `
  <div class="sec-h"><h2>The Climb · ${esc(scopeLabel())}</h2><span class="sub">What we said we'd do last week, what we did, and what we're committing to this week. ${inCount} of ${RS.length} commits in for the week ending ${fmtDate(w.key)}.</span></div>
  <div class="tiles">${cfg.measures.map((m) => { const t = tot[m.key]; return `<div class="tile ${ryg(t.d, t.c)}"><span class="stripe"></span><div class="l">${esc(mLabel(m))}</div><div class="v">${fmtM(m, t.d)} <small>/ ${fmtM(m, t.c)}</small></div><div class="s">this week's commit: <b>${fmtM(m, t.tc)}</b></div></div>`; }).join('')}</div>
